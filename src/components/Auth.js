...
const Auth = () => {
  ...
  const handleLogin = async (type) => {
    ...
    if (error) {
      ...
      setHelperText({
        error: false,
        text: "A email has been sent for verification",
      });
    }
  };
  ...
  const forgotPassword = async (e) => {
    ...
    if (email.trim() === null || email.trim() === "") {
      setHelperText({ error: true, text: "You must enter email" });
    } else {
      let { error } = await supabase.auth.api.resetPasswordForEmail(email);
      if (error) {
        console.log("Forogt Password Error: ", error.message);
      } else {
        setHelperText({
          error: false,
          text: "Password recovery email has been sent to you",
        });
      }
    }
  };
