...
const Home = ({ user }, props) => {
  ...
  const fetchTodos = async () => {
    ...
    else setTodos(todos.filter((x) => x.user_id === user.id));
  };
  ...
  return recoveryToken ? (
    ...
    <div className={"w-screen fixed flex flex-col min-h-screen bg-gray-50"}>
      ...
      >
        <span className={"text-2xl sm:text-4xl text-white font-sans"}>
          To do
        </span>
        <button
      ...
      <div className={"flex m-4 mt-0 h-10"}>
        <input
          ...
          className={
            "rounded-md bg-gray-200 border px-2 border-gray-300 w-full mr-4"
          }
        />
        <button
          onClick={addTodo}
